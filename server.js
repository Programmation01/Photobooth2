...
app.post('/api/email', async (req, res) => {
  ...
  try {
    ...
    // Fallback : renvoyer un lien direct même si l'email échoue
    res.status(500).json({
      error:       err.message,
...
server.listen(PORT, () => {
  ...
  console.log('╠══════════════════════════════════════════════╣');
  console.log(`║  Serveur  → ${BASE_URL.padEnd(33)}║`);
  console.log(`║  📱 Tél   → ${(BASE_URL + '/phone').padEnd(33)}║`);
  console.log(`║  📟 Table → ${(BASE_URL + '/tablet').padEnd(33)}║`);
